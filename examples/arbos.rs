...
const STYLUS_MAGIC_BYTES: &[u8] = &[0xEF, 0xF0, 0x00, 0x00];
...
async fn main() -> anyhow::Result<()> {
    ...
    let solidity_bytecode = Bytes::from(SOLIDITY_BYTECODE);
    ...
    cache_db.insert_account_info(
        ...
        AccountInfo {
            balance: U256::from(0),
            nonce: 0,
            code_hash: keccak256(solidity_bytecode.clone()),
            code: Some(Bytecode::new_legacy(solidity_bytecode.into())),
        },
    );

    let stylus_bytes = [
        Bytes::from(STYLUS_MAGIC_BYTES),
        Bytes::from(RUNTIME_BYTECODE),
    ]
    .concat();
    ...
    cache_db.insert_account_info(
        ...
        AccountInfo {
            balance: U256::from(0),
            nonce: 0,
            code_hash: keccak256(stylus_bytes.clone()),
            code: Some(Bytecode::new_legacy(stylus_bytes.into())),
        },
    );
    ...
    let mut evm = Evm::builder()
        ...
        .modify_tx_env(|tx: &mut TxEnv| {
            ...
            tx.transact_to = TxKind::Call(solidity_address);
        })
        .build();
